...
void vi_about(bool junk) {
	Msg_("About: coda-list (Version %s)",QVersion);
...
void abouto() {
 ...
 int main(int argc,char **args) {
	...
	static Obj makeobject(Self self,char *str) {
		...
		ei (cs_prefix((key="float:"),str)) {
			obj=obj_FromString(Class_Float,str+cs_length(key));
			if (!obj) Quit_("bad Float value: %s",OError());
			}
